# Como usar o PokeWGGrid (versão sem instalador)
...
2. Dê **dois cliques** no arquivo **Abrir PokeWGGrid** (`.vbs`). Ele abre o app **sem janela preta**.
...
**Quer um atalho na área de trabalho?** Clique com o **botão direito** no arquivo **Abrir PokeWGGrid** e escolha **Enviar para: Área de trabalho (criar atalho)**. Pronto, vira um clique só.

> O `iniciar.bat` continua na pasta e funciona igual, mas ele deixa uma janela preta aberta: se você fechar essa janela, o app fecha junto. Pelo **Abrir PokeWGGrid** isso não acontece.

**No Mac ou Linux:** abra o Terminal dentro da pasta e digite `bash iniciar.sh`.

## Passo 4: usar

1. Em cada quadradinho, entre na sua conta do pokewg.com (ou crie uma)
