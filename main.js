...
function logErro(origem, detalhe) {
  try {
    ...
    if (!errCabecalho) {
      errCabecalho = true;
      txt += `\n=== sessao de ${new Date().toLocaleString('pt-BR')} · PokeWGGrid v${app.getVersion()} · Electron ${process.versions.electron} · ${process.platform} ${require('os').release()} ===\n`;
    }
...
const startupLnk = () => path.join(startupDir(), 'PokeWGGrid.lnk');
const autoStartOn = () => { try { return process.platform === 'win32' && fs.existsSync(startupLnk()); } catch { return false; } };
function setAutoStart(on) {
  if (process.platform !== 'win32') return false;
  try {
    if (on) {
      const opts = { target: process.execPath, description: 'PokeWGGrid', appUserModelId: 'com.pokewggrid.app' };
...
ipcMain.handle('app:checkUpdate', () => new Promise((resolve) => {
  ...
  const req = https.get('https://raw.githubusercontent.com/SamuelRomani/PokeWGGrid/main/package.json', { headers: { 'User-Agent': 'PokeWGGrid' } }, (res) => {
...
function baixarArquivo(url, destino, saltos = 5) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.get(url, { headers: { 'User-Agent': 'PokeWGGrid' } }, (res) => {
...
const EXE_RELEASE_URL = 'https://github.com/SamuelRomani/PokeWGGrid/releases/download/latest/PokeWGGrid-Portable.exe';

// Dois jeitos de atualizar, dependendo de como o PokeWGGrid está rodando:
//  - código-fonte (npm start / Abrir PokeWGGrid.vbs): roda o atualizar.bat, que dá git pull ou
...
ipcMain.handle('app:runUpdate', async () => {
  ...
  if (!app.isPackaged) {
    ...
    const r = await dialog.showMessageBox({
      ...
      defaultId: 0,
      cancelId: 1,
      title: 'Atualizar o PokeWGGrid',
      message: 'Isso vai fechar o PokeWGGrid, baixar a versão mais nova e reabrir sozinho.',
    ...
    try {
      ...
      const batCopia = path.join(app.getPath('temp'), 'pokewggrid-atualizar.bat');
  ...
  const r = await dialog.showMessageBox({
    ...
    cancelId: 1,
    title: 'Atualizar o PokeWGGrid',
    message: 'Isso vai baixar o executável novo, fechar o PokeWGGrid, trocar o arquivo antigo pelo novo e reabrir sozinho.',
  ...
  const exeNovo = path.join(pasta, 'PokeWGGrid-Portable-novo.exe');
  const trocaBat = path.join(pasta, 'pokewggrid-troca-exe.bat');
  try {
    await baixarArquivo(EXE_RELEASE_URL, exeNovo);
  } catch (e) {
    logErro('atualizar', 'download do exe falhou: ' + String((e && e.message) || e));
    dialog.showErrorBox('Não consegui baixar a atualização', 'Confira sua internet e tente de novo. Nada foi mudado, o PokeWGGrid continua igual.');
  ...
  const scriptBat = [
    ...
    'goto fim',
    ':falhou',
    'echo Nao consegui trocar o executavel (ele pode ainda estar em uso). Abra o PokeWGGrid.exe antigo na mao.',
    'pause',
    ':fim'
  ].join('\r\n');
  try {
...
app.whenReady().then(() => {
  ...
  try { app.setAppUserModelId('com.pokewggrid.app'); } catch (e) { logErro('boot', 'appUserModelId: ' + e.message); } // notificacoes do Windows com o nome certo
